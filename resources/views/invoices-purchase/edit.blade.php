        ...
        <div class="my-auto">
            <div class="d-flex">
                ...
                <a href="{{route('purchase-invoice.show', $invoice->id)}}"><span
                        class="text-muted mt-1 tx-13 mr-2 mb-0"> > فاتورة رقم {{$invoice->id}} للمورد {{$invoice->supplier->name}} </span></a>
                <span
...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{route('purchase-invoice.update', $invoice->id)}}" method="POST" id="formData"
                        ...
                        <div class="row row-sm">
                            <div class="col-6">
                                <div class="form-group mg-b-0">
                                    ...
                                    <select name="supplier_id" class="w-75 clients">
                                        <option></option>
                                        @foreach($suppliers as $supplier)
                                            <option
                                                value="{{$supplier->id}}" {{($invoice->supplier_id == $supplier->id)? 'selected' : ''}}>{{$supplier->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                    ...
                                    <div class="card-body">
                                        <div class="table-responsive">
                                            <table class="table mg-b-0 text-md-nowrap ">
                                                ...
                                                @foreach($invoice->bellPurchase as $bellpurchase)
                                                    <tr>
                                                        <td class="w-25">
                                                            <select name="invoice[{{$loop->index}}][product_id]"
                                                                ...
                                                                @foreach($products as $product)
                                                                    <option
                                                                        value="{{$product->id}}" {{$bellpurchase->product->id == $product->id ? 'selected' : ''}}>{{$product->name}}</option>
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td><input type="number"
                                                                   ...
                                                                   value="{{$bellpurchase->purchase_price}}"
                                                            ></td>
                                                        <td><input type="number"
                                                                   name="invoice[{{$loop->index}}][amount]"
                                                                   placeholder="الكمية" class="amount"
                                                                   style="height: 33px"
                                                                   min="0" value="{{$bellpurchase->amount}}">
...
@section('js')
    ...
    <script>
        $(function () {
            ...
            pluginSelect2('.clients', 'أختر المورد');

...
@endsection
@section('title')
    فاتورة المورد  {{$invoice->supplier->name}}
@endsection
@section('page-header')
    <!-- breadcrumb -->
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto"><a href="{{route('purchase-invoice.index')}}">إدارة فواتير الشراء</a></h4><span
                    class="text-muted mt-1 tx-13 mr-2 mb-0"> > فاتورة رقم {{$invoice->id}} للمورد {{$invoice->supplier->name}} </span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
...
@section('content')
    <nav class="nav nav-pills flex-column flex-md-row mt-4">
        <a  class="nav-link bg-gray-200 text-dark ml-2 mb-1" href="{{route('purchase-invoice.edit', $invoice->id)}}"><i
                class="fa fa-edit fa-fw"></i> تعديل </a>
        <a class="nav-link bg-gray-200 text-dark ml-2 btnPrint mb-1" href="/invoice-purchase/print/{{$invoice->id}}"><i
                class="fa fa-print fa-fw" aria-hidden="true"></i> طباعة </a>
        <a class="nav-link bg-gray-200 text-dark ml-2 mb-1" href="{{route('payment-purchase.create', $invoice->id)}}"><i
        ...
        <li class="nav-item dropdown show" style="list-style: none">
            ...
            <div id="navbarDropdown{{$invoice->id}}"
                ...
                <a class="dropdown-item  text-dark ml-2 mb-1" href="{{route('invoice-refund-purchase.create',$invoice->id)}}"><i
        ...
        <div class="panel-body tabs-menu-body main-content-body-right border">
            <div class="tab-content">
                <div class="tab-pane active" id="tab4">
                    <iframe src="/invoice-purchase/print/{{$invoice->id}}" width="100%" height="600px" class="border-0"></iframe>
                </div>
                        ...
                        <div class="table-responsive mt-3">
                            ...
                            <table class="table table-striped mg-b-0 text-lg-nowrap ">
                                ...
                                @forelse ($payments as $payment)
                                    <tr class="text-center">
                                        ...
                                        <td>
                                            ...
                                            <a href="" class="btn btn-light p-1 delete_payment" data-title="حذف عملية دفع"
                                ...
                                <tr class="mt-4">
                                    ...
                                    <td colspan="2">{{number_format($invoice->total_due - $invoice->payments()->sum('paid'),2)}}</td>
                                </tr>
                                </tfoot>
                            </table>
...
@section('js')
    <script>
        ...
        $('.delete_invoice-refund').on('click', function () {
            let idMessage = $(this).data('id');
            $('#modelDelete').attr('action', '/invoice-refund-purchase/' + idMessage + '');
            $("#exampleModalLabel").text($(this).data('title'));
            $(".modal-body").text($(this).data('description'));
        });
        $('.delete_payment').on('click', function () {
            let idMessage = $(this).data('id');
            $('#modelDelete').attr('action', '/payment-purchase/' + idMessage + '');
            $("#exampleModalLabel").text($(this).data('title'));
            $(".modal-body").text($(this).data('description'));
        });

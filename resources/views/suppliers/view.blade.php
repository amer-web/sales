...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto"><a href="{{route('supplier.index')}}">إدارة الموردين</a></h4><span
                    class="text-muted mt-1 tx-13 mr-2 mb-0"> > المورد {{$supplier->name}}</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
            ...
            <div class=" tab-menu-heading">
                <div class="tabs-menu1">
                    ...
                    <ul class="nav panel-tabs main-nav-line">
                        ...
                        <li><a href="#tab5" class="nav-link" data-toggle="tab">الفواتير <span>({{$supplier->invoices->count()}})</span></a>
                        </li>
                        <li><a href="#tab6" class="nav-link" data-toggle="tab">المدفوعات <span>({{$supplier->payments()->where('paid', '!=',0)->count()}})</span></a>
                        </li>
            ...
            <div class="panel-body tabs-menu-body main-content-body-right border">
                <div class="tab-content">
                    <div class="tab-pane" id="tab4">
                        ...
                        <div class="row pt-3">
                            <div class="col-12 col-sm-6">
                                <h5>عدد الفواتير : <span
                                        class="text-primary  mr-3">{{$supplier->invoices->count() == 0 ? 'لا يوجد' : $supplier->invoices->count()}}</span>
                                </h5>
                            </div>
                            <div class="col-12 col-sm-6">
                                <h5>آخر فاتورة منشأة : <span class="text-primary mr-3">
                                                   @if($supplier->invoices->count() != 0)
                                            <a href="{{route('purchase-invoice.show',$supplier->invoices->last()->id)}}">فاتورة رقم  {{$supplier->invoices->last()->id}}</a>
                                        @endif
                                                </span></h5>
                            </div>
                        </div>
                        <div class="row pt-sm-3">
                            ...
                            <div class="col-12 col-sm-6">
                                <h5>آخر عملية دفع : <span class="text-primary mr-3">
                                                    @if($listPaid->count() != 0)
                                            <a href="{{route('payment-purchase.show',$listPaid->last()->id)}}">عملية دفع رقم  {{$listPaid->last()->id}}</a>
                                        @endif
                                                </span></h5>
                            </div>
                        </div>
                        ...
                        <div class="table-responsive mt-3">
                            ...
                            <table class="table table-striped mg-b-3 text-lg-nowrap ">
                                ...
                                <tr class="text-center">
                                    <td>{{number_format($supplier->invoices()->sum('total_due'),2)}}</td>
                                    <td>{{number_format($supplier->payments()->sum('paid'),2)}}</td>
                                    <td>{{number_format($supplier->invoices()->sum('total_due') - $supplier->payments()->sum('paid'),2 )}}</td>
                                </tr>
                                </tbody>
                            </table>
                                                            ...
                                                            <div id="navbarDropdown{{$invoice->id}}"
                                                                ...
                                                                <a href="{{route('purchase-invoice.show', $invoice->id)}}"
                                                                   class="dropdown-item" style="cursor: pointer"><i
                                                                        class="fa fa-eye fa-fw text-primary ml-1"></i>
                                                                    عرض </a>
                                                                <a href="{{route('purchase-invoice.edit', $invoice->id)}}"
                                                                   class="dropdown-item" style="cursor: pointer"><i
                                                                        class="fa fa-edit fa-fw ml-1"></i> تعديل </a>
                                                                <a class="nav-link text-dark ml-1 "
                                                                   style="font-size: 14px"
                                                                   href="{{route('payment-purchase.create', $invoice->id)}}"><i
                                                                        class="fa fa-credit-card fa-fw text-dark"></i>
                                                                    إضافة عملية دفع </a>
                                                                <a href="/invoice-purchase/print/{{$invoice->id}}"
                                                                   class="dropdown-item btnPrint"
                                                                   style="cursor: pointer"><i
                                                                        class="fa fa-print fa-fw text-info ml-1"
                                                                        aria-hidden="true"></i> طباعة </a>
                                                                <a href="" class=" delete_invoice dropdown-item"
                                                                   data-title="حذف فاتورة"
                                                                   data-description="هل تريد حذف فاتورة العميل ({{$invoice->supplier->name}})"
                    ...
                    <div class="tab-pane" id="tab6">
                        @if($supplier->payments()->where('paid', '!=', 0)->get()->isEmpty())
                                ...
                                <table class="table table-striped mg-b-0 text-lg-nowrap ">
                                    ...
                                    @forelse ($payments as $payment)
                                        <tr class="text-center">
                                            ...
                                            <td>{{$payment->paid}}</td>
                                            <td>
                                                <a href="{{ route('payment-purchase.show', $payment->id) }}" class=""><i
                                                        class="fa fa-eye fa-fw"></i></a>
                                                <a href="{{ route('payment-purchase.edit', $payment->id) }}"><i
                        ...
                        @else
                            <nav class="nav nav-pills flex-column flex-md-row mt-4">
                                <a class="nav-link bg-gray-200 text-dark ml-2 btnPrint mb-1 d-block"
                                   href="/supplier/print/{{$supplier->id}}"><i
                                        class="fa fa-print fa-fw" aria-hidden="true"></i> طباعة </a>
                            </nav>

                            <iframe src="/supplier/print/{{$supplier->id}}" width="100%" height="600px"

                ...
                <div class="card card-invoice">
                    <div class="card-body">
                        <div class="invoice-header">
                            <h1 class="invoice-title">فاتورة بيع مرتجعة</h1>
                        ...
                        <div class="row mg-t-20">
                            <div class="col-md">
                                <label class="tx-gray-600">فاتورة بيع مرتجعة من </label>
                                <div class="billed-to">
                                    <h6>{{$invoice_refund->invoice->client->name}}</h6>
                                    <p>العنوان : {{$invoice_refund->invoice->client->address}}<br>
                                        تليفون : {{$invoice_refund->invoice->client->phone}}<br>
                                        البريد الإلكترونى : {{$invoice_refund->invoice->client->email}}</p>
                                </div>
                            </div>
                            <div class="col-md">
                        ...
                        <div class="table-responsive mg-t-40">
                            <table class="table table-invoice border text-md-nowrap mb-0">
                                ...
                                @foreach($invoice_refund->salesRefund as $salesRefund)
                                    <tr class="font-weight-bold">
                                        <td>{{$loop->iteration}}</td>
                                        <td>{{$salesRefund->product->name}}</td>
                                        <td class="tx-center">{{$salesRefund->sale_price}}</td>
                                        <td class="tx-right">{{$salesRefund->amount}}</td>
                                        <td class="tx-right">{{$salesRefund->total}}</td>
                                    </tr>
                                @endforeach
                                ...
                                <tr class="font-weight-bold">
                                    ...
                                    <td class="tx-right" colspan="2">{{number_format($invoice_refund->salesRefund->sum('total'),2) }}</td>
                                </tr>

				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
                            ...
                            <a href="{{route('purchase-invoice.show', $invoice->id)}}"><span
                                    class="text-muted mt-1 tx-13 mr-2 mb-0"> > فاتورة رقم {{$invoice->id}} اللمورد {{$invoice->supplier->name}} </span></a>
                            <span
                                class="text-muted mt-1 tx-13 mr-2 mb-0"> > تعديل عملية دفع  </span>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
...
@section('content')
                ...
                <div class="row">
                    <div class="col-lg-12 col-md-12">
                        <div class="card">
                            <div class="card-body">
                                <form action="{{route('payment-purchase.update', $payment->id)}}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <div class="row row-sm">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select name="method_payment_id" class="w-75 method-payments">
                                                    ...
                                                    @foreach($method_payments as $method_payment)
                                                        <option value="{{$method_payment->id}}" {{$payment->method_payment_id == $method_payment->id ? 'selected' : ''}}>{{$method_payment->name}}</option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="col-6">
                                            <div class="form-group">
                                                ...
                                                <input class="form-control" name="paid" placeholder="المبلغ" type="number" autocomplete="off" value="{{$payment->paid}}">
                                                @error('paid')
                                                <span class="text-danger mb-3 d-block mt-1" style="font-size: 12px">{{$message}}</span>
                                                @enderror
                                            </div>
                                    ...
                                    <div class="row row-sm">
                                        ...
                                        <div class="col-12"><button class="btn btn-main-primary pd-x-20 mg-t-10" type="submit">تعديل عملية الدفع</button></div>
                                    </div>
                                </form>
                            </div>
                        </div>

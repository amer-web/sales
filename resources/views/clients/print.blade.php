                ...
                <div class="card card-invoice">
                    <div class="card-body">
                        ...
                        <div class="row mg-t-20">
                            <div class="col-md">
                                <label class="tx-gray-600">كشف حساب للعميل </label>
                                <div class="billed-to">
                                    <h6>{{$client->name}}</h6>
                                    <p>العنوان : {{$client->address}}<br>
                                        تليفون : {{$client->phone}}<br>
                                        البريد الإلكترونى : {{$client->email}}</p>
                                </div>
                            </div>
                            <div class="col-md">
                                ...
                                @foreach($accounts as $account)
                                    <tr class="text-center">
                                        ...
                                        <td>
                                            ...
                                            @elseif(isset($account->total_refund))
                                                فاتورة مرتجعة رقم {{$account->id}}
                                            @else
                                        ...
                                        <td>
                                            ...
                                            @elseif(isset($account->total_refund))
                                                <input name="" class="form-control text-center tx-bold price-total"
                                                       readonly
                                                       value="-{{$account->total_refund}}">
                                            @else
